...
import javax.swing.WindowConstants;
 public class Adjustments extends javax.swing.JFrame {
    ...
    public boolean adjustmentIsUsed = false;
    
    home hm;
    Ledger ld;
    private ShowInitialDetails SI;
    JEAndAdjusmentSC JEA;
    public Adjustments(home hm, Ledger ld) {
    ...
    private boolean confirmAction(String message) {
     ...
     private boolean isProjectSaved(int projectId) {
         String query = "SELECT is_saved FROM adjustments WHERE project_id = ?";
...
//           do nothing
    ...
    private boolean saveJournalEntry() {
        ...
        try (Connection conn = DriverManager.getConnection(
            ...
            String selectSql = "SELECT adjustment_id FROM adjustments WHERE project_id = ? ORDER BY adjustment_id ASC";
            try (PreparedStatement psSelect = conn.prepareStatement(selectSql)) {
                psSelect.setInt(1, projectId);
                try (ResultSet rs = psSelect.executeQuery()) {
                    while (rs.next()) {
                        existingIds.add(rs.getInt("adjustment_id"));
                    }
                }
            }

            // Step 2: Prepare statements for update and insert
            String updateSql = "UPDATE adjustments SET adjustment_date=?, transaction_no=?, account_name=?, debit=?, credit=?, description=? WHERE adjustment_id=?";
            String insertSql = "INSERT INTO adjustments (project_id, adjustment_date, transaction_no, account_name, debit, credit, description) VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            if (existingIds.size() > model.getRowCount()) {
                for (int i = model.getRowCount(); i < existingIds.size(); i++) {
                    String deleteSql = "DELETE FROM adjustments WHERE adjustment_id =?";
            ...
            JOptionPane.showMessageDialog(null, "Adjustments saved successfully!");
            return true;
        } catch (SQLException e) {
        ...
        try {
            ...
            if(checkAndHighlightUnbalancedAccounts() == true) {
                ...
                if(successful) {
                    try (Connection conn = DriverManager.getConnection(
                        "jdbc:mysql://localhost:3306/accountingcycle", "root", "123456789")) {
                        String updateSql = "UPDATE adjustments SET is_saved = TRUE WHERE project_id = ?";
                        PreparedStatement ps = conn.prepareStatement(updateSql);
                        ps.setInt(1, projectId);
                        ps.executeUpdate();
                    }
                    isProjectSaved = true;
                    adjustmentIsUsed = adjustmentIsSaved(projectId);
    ...
    private boolean adjustmentIsSaved(int projectId) {
        String checkQuery = "SELECT adjustment_is_used FROM projects WHERE project_id = ?";
        String updateQuery = "UPDATE projects SET adjustment_is_used = TRUE WHERE project_id = ?";
        ...
        try (Connection conn = DriverManager.getConnection(
            ...
            try (PreparedStatement checkStmt = conn.prepareStatement(checkQuery)) {
                ...
                try (ResultSet rs = checkStmt.executeQuery()) {
                    if (rs.next()) {
                        boolean alreadySaved = rs.getBoolean("adjustment_is_used");
                        if (alreadySaved) {
                            return true; // Already saved, no need to update
                        }
                    } else {
    ...
    private boolean loadAdjustments(int projectIdKey) {
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
        model.setRowCount(0); // Clear existing rows

        String selectSql = "SELECT adjustment_date, transaction_no, account_name, debit, credit "
                + "FROM adjustments WHERE project_id = ? ORDER BY transaction_no";

        String maxSql = "SELECT COALESCE(MAX(transaction_no), 0) AS max_trans FROM adjustments WHERE project_id = ?";
        ...
        try (Connection conn = DriverManager.getConnection(
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    Date date = rs.getDate("adjustment_date");
                    ...
                    if (!creditStr.isEmpty()) accTitle = "     " + accTitle;

                    model.addRow(new Object[] { dateFormatted, accTitle, transNo, debitStr, creditStr });
                }
            }

            // Determine next transacNo from DB (robust even if no rows)
            psMax.setInt(1, projectIdKey);
            try (ResultSet rs2 = psMax.executeQuery()) {
                int maxTrans = 0;
                if (rs2.next()) maxTrans = rs2.getInt("max_trans");
                transacNo = maxTrans + 1;
            }

            // UI adjustments
            if (!anyRows) {
                // optional: show info, disable save/delete etc.
                System.out.println("No adjustments found for project_id " + projectIdKey);
                // jButtonSave.setEnabled(false); // example
            } else {
                // jButtonSave.setEnabled(true);
            }

            System.out.println("Adjustments loaded for project_id " + projectIdKey + ", next transacNo=" + transacNo);
            return anyRows;

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this,
                    "Error loading adjustments: " + e.getMessage(),
                    "Database Error",
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Adjustments(null, null).setVisible(true);
            }
        });
    }
